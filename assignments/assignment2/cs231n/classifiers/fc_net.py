...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        cache_affine, cache_bn, cache_ln, cache_relu, cache_dropout = [], [], [], [], []
        ...
        # calculate regularization weight
        reg_weights = 0
        for i in range(self.num_layers):
            reg_weights += np.sum(self.params['W' + str(i+1)] ** 2)
        ...
        # calculate gradients
        dout, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dout, cache_affine[self.num_layers-1])

        for i in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
                dout = dropout_backward(dout, cache_dropout[i-1])
            dout = relu_backward(dout, cache_relu[i-1])
            if self.normalization == 'batchnorm':
                dout, grads['gamma' + str(i)], grads['beta' + str(i)] = batchnorm_backward_alt(dout, cache_bn[i-1])
            if self.normalization == 'layernorm':
                dout, grads['gamma' + str(i)], grads['beta' + str(i)] = layernorm_backward(dout, cache_ln[i-1])
            dout, grads['W' + str(i)], grads['b' + str(i)] = affine_backward(dout, cache_affine[i-1])
