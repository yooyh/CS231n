...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # layer 2 : word embedding
        x, cache2 = word_embedding_forward(captions_in, W_embed)

        # layer 3 : RNN or LSTM
        if self.cell_type == 'rnn':
          x, cache3 = rnn_forward(x, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
          x, cache3 = lstm_forward(x, h0, Wx, Wh, b)

        # layer 4 : temporal affine
        x, cache4 = temporal_affine_forward(x, W_vocab, b_vocab)

        # layer 5 : temporal softmax
        loss, dout = temporal_softmax_loss(x, captions_out, mask)

        # backpropagation
        # layer 4 : temporal affine
        dout, dW, db = temporal_affine_backward(dout, cache4)
        grads['W_vocab'], grads['b_vocab'] = dW, db

        # layer 3 : RNN or LSTM
        if self.cell_type == 'rnn':
          dx, dh0, dWx, dWh, db = rnn_backward(dout, cache3)
        elif self.cell_type == 'lstm':
          dx, dh0, dWx, dWh, db = lstm_backward(dout, cache3)
